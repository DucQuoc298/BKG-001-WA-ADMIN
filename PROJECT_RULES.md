...
## 2. Cấu trúc Redux State theo Module (Bắt buộc áp dụng cho Module mới)

Mọi Redux Slice trong dự án phải tuân theo kiến trúc **phân cấp theo trang** như sau:

```
IXxxState {
  loading: boolean;       // Cờ loading chung khi gọi API
  saving: boolean;        // Cờ saving khi lưu dữ liệu
  error: any;             // Lỗi API chung của module
  message: any;           // Thông báo chung (success/error message)
  xxx: {                  // Nhánh chứa state của các trang con (đặt tên theo module, VD: home, customer)
    list?: {              // (Tùy chọn) State của trang Danh Sách
      searchKeyword: string;
      // ... các filter, pagination
    };
    form?: {              // (Tùy chọn) State của trang Form
      data: XxxFormFields;          // Chỉ chứa các giá trị trường nhập liệu
      dirtyFields: Record<string, any>; // Các trường đã bị thay đổi
      formMode: EFormMode;           // Chế độ hiển thị (VIEW, FORM, LIST)
    };
    // ... các trang ngang cấp khác nếu có
  };
}
```

**Ví dụ cụ thể (module Home):**
```typescript
export interface IHomeState {
  ...
  home: {
    ...
    form: {
      data: HomeFormFields;          // Giá trị các trường nhập liệu
...
**Quy tắc tách biệt `FormFields` vs `FormState`:**
- **`XxxFormFields`**: Chỉ chứa các giá trị trường nhập liệu. Được dùng làm kiểu dữ liệu cho `useForm<XxxFormFields>()` của React Hook Form.
- **`XxxFormState`**: Bao gồm `data: XxxFormFields`, `dirtyFields`, và `formMode`. Đây là toàn bộ snapshot được lưu/khôi phục qua `useReduxFormSync`.

**Lý do tách biệt này:**
- `dirtyFields` và `formMode` là metadata, không phải giá trị trường — để chung sẽ gây ra lỗi type khi truyền vào `useForm`.
- Cho phép nhiều form nhỏ trong cùng một trang đều có `dirtyFields` riêng biệt.
- Dễ extend thêm state trong tương lai mà không ảnh hưởng tới kiểu dữ liệu React Hook Form.
