...
> **Lưu ý cho AI Agent:** Đọc toàn bộ tài liệu này trước khi viết bất kỳ dòng code nào. Mọi quy ước trong tài liệu này đều bắt buộc. Xem thêm [project_rules.md](/docs/project_rules.md) và [agents.md](/docs/agents.md) để biết toàn cảnh dự án.
...
Mở file [form.ts](/src/types/form.ts) và thêm định danh cho form vào enum `IFormKey`:
```typescript
...
Mở file [createStore.ts](/src/store/createStore.ts) và import reducer mới để chèn vào `rootReducer`.
...
> Xem chi tiết kiến trúc, SDK API reference và sequence diagram tại [runtime_plugin_flow.md](/docs/runtime_plugin_flow.md).
...
type MyFormFields = { name: string; email: string };

export const createPluginComponent = definePlugin(({ react: React, sdk }) => {
  // BẮT BUỘC: Lấy mọi thứ từ sdk, KHÔNG import trực tiếp
  ...
  const defaultValues: MyFormFields = { name: '', email: '' };
  ...
  function MyPlugin() {
    ...
    // Đồng bộ form ↔ Redux (pluginId tự động scope)
    ...
    return (
      <ContainerWrapper>
        <FormProvider {...methods}>
          <MainCard title="My Plugin">
            <Stack spacing={2}>
              <TextField label="Name" {...register('name')} value={formValues.name} />
              <Button text="Submit" onClick={handleSubmit(console.log)} />
            </Stack>
          </MainCard>
        </FormProvider>
      </ContainerWrapper>
...
> - `export const createPluginComponent`: Factory function nhận `{ react, sdk }` và trả về React Component.

> **Cấm import thư viện ngoài**: KHÔNG import `@mui/material`, `react`, `react-hook-form`, `react-redux` trực tiếp từ `node_modules`. Dùng các API được tiêm qua `sdk` để giữ bundle `.mjs` siêu nhẹ.
...
```json
{
  ...
  "moduleUrl": "./ten-plugin.mjs",
  "enabled": true,
  "sidebar": true
}
```

> **Không cần thay đổi host app**: Host sử dụng generic runtime tự động tạo SDK cho mọi plugin. Không cần tạo runtime declaration, không cần sửa `AppPlugin.tsx`, không cần đăng ký store.

---

## 5. Giao tiếp Chéo Tab (BroadcastChannel)

Dự án hỗ trợ đồng bộ hóa trạng thái giữa các tab trình duyệt qua `BroadcastChannel` (được bọc trong `src/services/utils/broadcast.ts` và hook `useBroadcastChannel`).
...
Mọi sự kiện chéo tab **bắt buộc** phải được khai báo trong `BroadcastEventTypes` tại [broadcast.ts](/src/services/utils/broadcast.ts). Không sử dụng magic strings.
...
- [ ] Có import trực tiếp `react`/`@mui/material`/`react-hook-form` từ `node_modules` thay vì `sdk` không?
- [ ] Đã thêm vào `manifest.json` với `enabled: true` chưa?
- [ ] Form có sử dụng `sdk.hooks.useReduxFormSync` để persist state không?
- [ ] Có sử dụng `sdk.store.getPluginFormState()` / `updatePluginForm()` đúng cách không?
