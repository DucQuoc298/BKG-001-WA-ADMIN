...
const Error403 = () => {
  ...
  return (
    <MaintenanceWrapper>
      <Stack>
        <img src={Error403Image} alt="403 Error" style={styles.image} />
      </Stack>
      <Stack direction={"column"} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
        <Typography variant="h1" sx={{ ...styles.title }}>{t('maintenance.403.title')}</Typography>
        <Typography variant="body1" sx={{ ...styles.body }}>{t('maintenance.403.message')}</Typography>
        <Button
          variant="contained"
          sx={{ ...styles.button }}
          onClick={() => navigate('/')}
